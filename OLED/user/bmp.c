		...
		{				 
			while(count<readlen)  //读取一簇1024扇区 (SectorsPerClust 每簇扇区数)
		    {
				if(color_byte==3)   //24位颜色图
				{
					...
					{
						case 0:				  
							color[0]=bmpbuf[count]; //B
							break ;	   
						case 1: 	 
							color[1]=bmpbuf[count];//G
							break;	  
						case 2 : 
							color[2]=bmpbuf[count];//R	  
							break ;			
					}   
				}
				else if(color_byte==2)  //16位颜色图
				{
					switch(rgb)
					{
						case 0 : 
							...
							{
								color[2]=bmpbuf[count]<<1;	 	//R
								color[1]=bmpbuf[count]<<6; //G
							}else		//RGB:5,6,5
							{
								color[2]=bmpbuf[count];	 	//R
								color[1]=bmpbuf[count]<<5; //G
							}  
							break ;   
						case 1 : 			  			 
							if(biCompression==BI_RGB)//RGB:5,5,5
							{
								color[1]|=bmpbuf[count]>>2;  //R,G
								color[0]=bmpbuf[count]<<3;
							}else  		//RGB:5,6,5
							{
								color[1]|=bmpbuf[count]>>3;  //R,G
								color[0]=bmpbuf[count]<<3;
							}  									 
							break ;	 
					}		     
				...
				{	
					...
					{	
						realx=(x*picinfo.Div_Fac)>>13;//x轴实际值		 				 	  	       
							pic_phy.draw_point(realx+picinfo.S_XOFF-1,realy+picinfo.S_YOFF-1,color);//显示图片	
							//POINT_COLOR=color;		 
