...
{
	pic_phy.read_point=ili9325_read_point;  		//读点函数实现
	pic_phy.draw_point=ili9325_draw_point;	//画点函数实现
	pic_phy.fillcolor=piclib_fill_color;  	//颜色填充函数实现 

	picinfo.lcdwidth=X_MAX;	//得到LCD的宽度像素
	picinfo.lcdheight=Y_MAX;//得到LCD的高度像素
...
{	
	u8	res;//返回值
	u8 temp;	
	if((x+width-1)>picinfo.lcdwidth)return PIC_WINDOW_ERR;		//x坐标超范围了.
	if((y+height-1)>picinfo.lcdheight)return PIC_WINDOW_ERR;		//y坐标超范围了.  
	//得到显示方框大小	  	 
